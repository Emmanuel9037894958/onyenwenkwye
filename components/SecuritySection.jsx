...
export default function SecuritySection() {
  const securityItems = [
    {
      id: 1,
      icon: <Shield className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 text-white" />,
      title: "Regulated Activities",
      description: "FSO license No. 1426969",
    },
    {
      id: 2,
      icon: <Lock className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 text-white" />,
      title: "Negative Balance Protection",
      description: "Your funds are fully secure",
    },
    {
      id: 3,
      icon: <Users className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 text-white" />,
      title: "Financial Commission Participant",
      description: "Trusted by global standards",
    },
    {
      id: 4,
      icon: <Award className="w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 text-white" />,
  ...
  return (
    <section className="bg-orange-500 py-16 w-full">
      {/* ✅ Removed mx-auto, added w-full and px */}
      <div className="w-full text-center px-2 sm:px-4 md:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {securityItems.map((item) => (
            <div key={item.id} className="flex flex-col items-center text-center">
              ...
              <p className="text-sm sm:text-base md:text-lg text-white">{item.description}</p>
            </div>
          ))}
        </div>
